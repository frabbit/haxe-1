...
class TestByteArray extends Test {
	...
	function test() {
		...
		eq(b.get(7),0);
		// readString
		var bs = haxe.io.ByteArray.ofString("One é accent");
		...
		**/
		...
		for( s1 in strings )
			for( s2 in strings ) {
				var c = haxe.io.ByteArray.ofString(s1).compare(haxe.io.ByteArray.ofString(s2));
	...
	function testBytesDataEquality () {
		var b1 = haxe.io.ByteArray.ofString("AB");
		var x = b1.getData();
		var b2 = haxe.io.ByteArray.ofData(x);
