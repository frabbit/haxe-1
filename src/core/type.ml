 ...
 *)
 ...
 let log_type' prefix (t:t) ?(force=false) =
	 let enabled = (!log_enabled) || force in
 ...
 let log_type prefix (t:t) =
	 log_type' prefix t ~force:false
 ...
 let log_object prefix t f =
	 ...
	 if enabled then
		 Printf.printf ("%s: %s%s%s\n")
		 (log_pad prefix)
		 log_code_light_green (f t) log_code_reset;
	 ()

 let log_message prefix t = log_object prefix t (fun s -> s)
 ...
 *)
 ...
 and is_in_type (t:t) = match follow1 t with
 ...
 and unapply_right (tl:t list) =
	 match (unapply_right_1 (List.rev tl)) with
	 | (x, tl) -> x, List.rev tl

 and unapply_right_1 (tl:t list) = match (tl) with
	 | [] -> None, []
	 | t :: tl  ->
		 (if is_in_type t then
			 let x = unapply_right_1 tl in
			 (fst x), (!t_in)::(snd x)
		 else
			 (Some t), (!t_in)::tl)
 ...
 and to_apply_option t =
	 ...
	 | TEnum (e, tl) ->
		 ...
		 handle_params tl mk
	 | TAbstract (e, tl) ->
		 let mk x = TAbstract (e,x) in
		 handle_params tl mk
	 | TInst (e, tl) ->
		 let mk x = TInst (e,x) in
		 handle_params tl mk
	 | TType (e, tl) ->
		 let mk x = TType (e,x) in
		 handle_params tl mk
	 | TFun(args, ret) ->
		 let mk x = match List.rev x with
			 | ret::tl ->
				 let tl = List.rev tl in
				 let args = List.map2 (fun t (n,o,_) -> (n,o,t)) tl args in
				 TFun(args, ret)
			 | [] -> assert false
		 in
		 let args = List.map (fun (_,_,t) -> t) args in
 ...
 *)
 ...
 and normalize_of_type t =
	 ...
	 let rec loop t =
		 ...
		 | TAbstract({a_path=[],"-Of"}, [tm; tp]) when is_of_type (follow1 tp) ->
			 begin match follow1 tp with
				 | TAbstract({a_path=[],"-Of"}, [tm2; tp]) ->
		 ...
		 (*| TAbstract({a_path=[],"-Of"}, [tm; TAbstract({a_path=[],"-Of"}, [tm2; tp])]) ->
			 (*log_message "normalize_of_type" "of-of";*)
			 let tm =  mk_of tm tm2 in
			 loop (mk_of tm (normalize_of_type tp))*)
 ...
 and reduce_apply_rec t =
	 let rp = List.map reduce_apply_rec in
	 let t = match follow t with
	 | TInst(c,tl) ->
		 TInst(c, rp tl )
	 | TEnum(en,tl) ->
	 ...
	 | TType(ta, tl) ->
		 TType( ta, rp tl)
	 | TFun(p, r) ->
		 TFun( List.map (fun (s, b, t) -> (s, b, reduce_apply_rec t)) p, reduce_apply_rec r )
	 | _ ->
		 t
	 in
	 from_apply t
		 ...
		 with Unify_error l ->
			 try
				 let rec loop t1 t2 =
					 ...
					 | a,b when is_in_type a ->
						 ()
					 | a,b when (is_apply_type t1) || (is_apply_type t2) ->
						 (try
							 unify_apply t1 t2
						 with Unify_error l ->
							 with_variance (type_eq EqRightDynamic) (from_apply t1) (from_apply t2))
					 ...
					 | _ ->
						 ...
						 with Unify_error l ->
							 begin try
								 with_variance (type_eq EqRightDynamic) (reduce_apply_rec t1) (reduce_apply_rec t2)
							 with Unify_error l ->
								 raise (Unify_error l)
							 end
						 end
